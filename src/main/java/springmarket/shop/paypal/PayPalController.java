package springmarket.shop.paypal;
...
public class PayPalController {
    ...
    public String buy(Model model, @PathVariable(name = "orderId") Long orderId) {
        try {
            springmarket.shop.entities.Order order = orderService.findById(orderId);

            Payer payer = new Payer();
            payer.setPaymentMethod("paypal");
            RedirectUrls redirectUrls = new RedirectUrls();
            redirectUrls.setCancelUrl("http://localhost:8189/shop/paypal/cancel");
            redirectUrls.setReturnUrl("http://localhost:8189/shop/paypal/success/" + order.getId());
            ...
            transaction.setDescription("Покупка в Shop");
    ...
    public String success(HttpServletRequest request, HttpServletResponse response, Model model, @PathVariable(name = "orderId") Long orderId) {
        try {
            ...
            if (executedPayment.getState().equals("approved")) {
                ...
                orderService.updateOrder(orderId, "payed");
